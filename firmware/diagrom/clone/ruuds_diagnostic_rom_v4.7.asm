...
AbsoluteAddress	equ	InvScreenRAM+43		; 43, 44 and 45 for an absolute address (eg. 0D4A34 hex). Populated by subroutine 'CalcAbsFromSegOff'
...
InitCom1:
	...
	; Note: At this point in time, in my serial configuration, a strange character appears on the serial terminal.
	...
	; If not here, in my serial configuration, the "33 0 2" appears as strange characters.
	xor	cx,cx
	loop	$
...
InitMdaCga:
	...
	mov	si,Tbl_dataMDA
	mov	cx,16
	xor	bx,bx			; BL is register counter
.L10:	mov	al,bl
	out	dx,al			; Select register
	lodsb				; Copy contents of [DS:SI] into AL, then increment SI
	inc	dl
...
; ****************************************************************************
; * Set the correct video mode, and make the cursor invisible.
; *
; * At this point in time, we do not know which is present, MDA or CGA.
; * Maybe neither (e.g. user is using a serial port to see checkpoints).
; * Just assume that an MDA or CGA is fitted.
...
SetVideoMode:
	...
	mov	dx,MDA_ctrl		; MDA control register
	mov	al,29h
	out	dx,al
	...
	mov	al,ch			; Send CH, CL through CRT register AL
	mov	dx,MDA_addr
	xchg	al,ah
	...
	; Set the correct CGA mode.
	mov	dx,CGA_ctrl		; CGA control register
	mov	al,29h
	out	dx,al
	mov	dx,CGA_pal		; CGA palette register
	mov	al,30h
	out	dx,al
...
; *
; * With the two 'RAM parity error' latches disabled, both latches are expected to be in a reset state.
...
.EXISTS:
	...
	; "RAM was found" = four test values revealed a bad bit count between 0 and 3
...
; * For example, in most cases, it will throw an error if a 4164 type RAM chip has been accidentally put into a socket that requires a 41256 type.
...
; *        90 seconds allows for a buffer, and the fact that someone may have a motherboard that holds contents for longer.
; *
; ****************************************************************************
TestRamRefr:
...
;
;  RETURNS: - DI is the offset in MDA/CGA video memory.
;
; DESTROYS: {nothing}
;
; ****************************************************************************
...
;
;  RETURNS: Carry flag, clear (NC) if PASS, set (C) if FAIL.
;
...
;   INPUTS: {nothing}
;
;  RETURNS: Carry flag, clear (NC) = interrupt occurred, set (C) = no interrupt
;
; DESTROYS: BL, CX, ES
;
; ****************************************************************************
...
;             Parameters:
;                1st byte: rowAX, BX, CX, DI, DX, ES, SI
;                2nd byte: column
;                The text starts at the 3rd byte
;
;  RETURNS: {nothing}
...
;
;  RETURNS: Carry flag, clear (NC) = SUCCESS, set (C) = ERROR
;
; DESTROYS: AX, CX, DX
;
; ****************************************************************************
...
SendBadAddressToCom1:
	...
	; Variable 'AbsoluteAddress' is 3 bytes, e.g. address 010000, 6 digits.
...
;                 The text starts at the 3rd byte
;
;  RETURNS: SI points to text starting at the 3rd byte.
;
...
;                 The text starts at the 3rd byte
;
;  RETURNS: DI is offset into MDA/CGA video RAM.
;
; DESTROYS: AX, CH, DI
;
; ****************************************************************************
...
; ****************************************************************************
; There was a failure of a RAM related test (address or data).
; Display details, then halt the CPU.
;
...
;             - DX contains the offset of the address.
;
;    RETURNS: Result placed in our 3-byte variable named 'AbsoluteAddress'
;
;   DESTROYS: BX, CL
;
; ****************************************************************************
...
TxtTitle:
	db  "Ruud's Diagnostic ROM for PC/XT     Version: 4.7  ("
